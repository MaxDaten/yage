...
void main()
{
    ...
    for (int i=1; i<3; i++) {
        texColor += texture( TextureSampler1, SamplingUV0 + vec2(offsets[i] * TextureSize0.z, 0.0) ) * weights[i];
        texColor += texture( TextureSampler2, SamplingUV0 - vec2(offsets[i] * TextureSize0.z, 0.0) ) * weights[i];
    }
